...
namespace Funcify
{
    public class ResizeImageFunction
    {
        ...
        public async Task Run([QueueTrigger("funcifyqueue", Connection = "Storage")] QueueMessage message)
        {
            try
            {
                _logger.LogInformation($"Queue trigger function processing: {message.MessageText}");
                ...
                _logger.LogInformation($"Processing product with ID: {queueMessage.ProductId}, Image URL: {queueMessage.UnprocessedImageUrl}, File Name: {queueMessage.FileName}");
